...
# The SLAM mini-project was developed by Joseph Moghadam (jmoghadam@berkeley.edu).


"""
slam.py is a simplified pacman version of the simultaneous localization 
and mapping problem. Pacman does not know his own position, nor the positions
of the walls. His goal is to simultaneously map the walls and track his own
trajectory.

To start a human controlled game, type 'python pacman.py' from the command line.
This will run a default implementation of inference, which knows essentially 
nothing. When you overwrite it, this will run your version.
The keys to move around are 'a', 's', 'd', and 'w' to move (or arrow keys).
Beware! Pacman will only move as you say 90% of the time, and will act randomly
otherwise.  
...
import sys, util, types, time, random, layout, os, pdb
...
def getNoisyDistance(pos1, pos2):
    ...
    return max(0, min(distance, distance + util.sample(SONAR_NOISE_PROBS, SONAR_NOISE_VALUES)))
...
class SlamRules:
    ...
    def newGame( self, layout, pacmanAgent, ghostAgents, display, maxMoves= 10 ):
        agents = [pacmanAgent] + ghostAgents
        initState = GameState()
        initState.initialize( layout, len(ghostAgents))
        startPos = initState.getPacmanPosition()
        wallPrior = calculateWallPrior(initState.getWalls())
        pacmanAgent.tellGameInfo(startPos, initState.getWalls().width, initState.getWalls().height, wallPrior, initState.getLegalPositions())
...
def calculateWallPrior(walls):
    """Calculate the probability that 
    a square is a wall, prior to any
    observations.
    """
    total_walls = 0.0
    total_spaces = 0.0
    for i in xrange(walls.width):
        for j in xrange(walls.height):
            total_spaces += 1
            if walls[i][j]:
                total_walls += 1
    return total_walls/total_spaces
...
def readCommand( argv ):
    ...
    parser.add_option('-p', '--pacman', dest='pacman',
                      ...
                      metavar='TYPE', default='SLAMKeyboardAgent')
